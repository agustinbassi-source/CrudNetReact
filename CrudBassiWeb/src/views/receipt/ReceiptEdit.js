...
const renderErrors = () =>
  ...
  useEffect(() => {
    (async () => {
      const response = await fetch(process.env.REACT_APP_API_URL + `api/Receipts/${id}`);
      const data = await response.json();
      console.log(data.data);

      for (let index = 0; index < data.data.receiptDetail.length; index++) {
        data.data.receiptDetail[index].rowId = index;
      }
  ...
  const saveChanges = (e) => {
    ...
    const response = fetch(process.env.REACT_APP_API_URL + `api/Receipts/${id}`, {
